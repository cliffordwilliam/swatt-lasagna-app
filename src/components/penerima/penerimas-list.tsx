...
const PenerimasList: FunctionComponent<PenerimasListProps> = ({ getToken }) => {
  const [isLoading, setLoading] = useState(false);
  const [penerimas, setPenerimas] = useState([] as Penerima[]);
  ...
  useEffect(() => {
    const getPenerimas = async () => {
      try {
        ...
        if (response?.data) {
          const penerima: Penerima[] = response.data.data;
          setPenerimas(penerima);
        ...
        toast({
          title: "Penerimas Gagal",
          description:
            "Gagal mengambil penerimas. Harap periksa kredensial Anda dan coba lagi.",
        });
        router.push("/sign-in");
      } finally {
        setLoading(false);
  ...
  const handleDelete = async (value: string) => {
    try {
      ...
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/penerimas/${value}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (response?.data) {
        toast({
          ...
          description: "Anda telah berhasil menghapus penerima.",
        });
      ...
      toast({
        ...
        description:
          "Gagal menghapus penerima. Harap periksa kredensial Anda dan coba lagi.",
      });
      router.push("/sign-in");
    } finally {
      setLoading(false);
  ...
  if (penerimas && penerimas.length !== 0) {
    return (
      <>
        {renderTopInputs()}
        <div className="grid sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 gap-4 p-4">
          {penerimas.map((penerima) => (
            <Card key={penerima.id}>
              <CardHeader className="space-y-2 p-6">
                ...
                <CardDescription>{penerima.alamat}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4 border-t px-6 py-8">
                <div className="text-4xl font-bold">{penerima.noHp}</div>
              </CardContent>
              <CardFooter className="flex gap-2">
                <Dialog>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>Yakin mau menghapus penerima?</DialogTitle>
                      <DialogDescription>
                        Sekali di hapus penerima ini akan hilang selamanya.
                      </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                      <Button
