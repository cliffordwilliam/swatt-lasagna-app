...
const UpdatePembeli = ({ data, getToken }: UpdatePembeliProps) => {
  ...
  async function onSubmit(value: z.infer<typeof formSchema>) {
    try {
      ...
      const response: AxiosResponse<CreatePembeliResponse> = await axios.patch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/pembelis/${data.id}`,
        value,
        {
          headers: {
      ...
      if (response?.data) {
        toast({
          ...
          description: "Anda telah berhasil mengubah pembeli.",
        });
        route.refresh();
      } else {
      ...
      toast({
        title: "Pembeli Gagal",
        description:
          "Gagal mengubah pembeli. Harap periksa kredensial Anda dan coba lagi.",
      });
    } finally {
      setLoading(false);
    }
      ...
      <Card className="w-full">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <CardHeader>
              <CardTitle className="text-2xl">Ubah Pembeli</CardTitle>
              <CardDescription>
